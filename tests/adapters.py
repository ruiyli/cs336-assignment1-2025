...
def run_multihead_self_attention(
    ...
    # 不带 RoPE 的版本: theta/max_seq_len 随便给, 因为不用 RoPE
    layer = CausalMultiHeadSelfAttention(d_model=d_model, num_heads=num_heads, max_seq_len=4096, theta=10000.0, use_rope=False, device=in_features.device)
...
def run_multihead_self_attention_with_rope(
    ...
    """
    from cs336_basics.transformer import CausalMultiHeadSelfAttention
    layer = CausalMultiHeadSelfAttention(d_model=d_model, num_heads=num_heads, max_seq_len=max_seq_len, theta=theta, use_rope=True, device=in_features.device)
    layer.load_state_dict({
        "q_proj.weight": q_proj_weight,
        "k_proj.weight": k_proj_weight,
        "v_proj.weight": v_proj_weight,
...
def run_transformer_block(
    ...
    layer = TransformerBlock(d_model=d_model, num_heads=num_heads, d_ff=d_ff, max_seq_len=max_seq_len, theta=theta, device=in_features.device)
...
def run_transformer_lm(
    ...
    model = TransformerLM(vocab_size=vocab_size, context_length=context_length, d_model=d_model, num_layers=num_layers, num_heads=num_heads, d_ff=d_ff, rope_theta=rope_theta, device=in_indices.device)
...
def run_gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm: float) -> None:
    ...
    """
    from cs336_basics.optimizer import clip_gradient
